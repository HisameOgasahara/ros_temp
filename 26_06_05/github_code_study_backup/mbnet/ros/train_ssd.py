...
if __name__ == '__main__':
    ...
    if args.net == 'vgg16-ssd':
        ...
        if hasattr(config, "set_image_size"):
            config.set_image_size(args.resolution)
        elif args.resolution != config.image_size:
            raise ValueError(f"{args.net} only supports resolution {config.image_size} with this config.")

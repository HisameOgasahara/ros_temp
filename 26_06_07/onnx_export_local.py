...
if args.net == 'vgg16-ssd':
    ...
    if hasattr(mobilenetv1_ssd_config, 'set_image_size'):
        mobilenetv1_ssd_config.set_image_size(args.resolution)
    else:
        if args.resolution != mobilenetv1_ssd_config.image_size:
            raise ValueError(
                f"this mobilenetv1_ssd_config only supports "
                f"{mobilenetv1_ssd_config.image_size}x{mobilenetv1_ssd_config.image_size}"
            )
...
dummy_input = torch.randn(args.batch_size, 3, args.resolution, args.resolution).to(device)
